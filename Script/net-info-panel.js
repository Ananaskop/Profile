...
function getIP() {
    ...
    if (!v4 && !v6) {
      ...
      if (v4?.primaryRouter && getSSID()) info.push(`路由网关IPv4 @ ${v4?.primaryRouter}`);
      if (v6?.primaryRouter && getSSID()) info.push(`路由网关IPv6 @ ${v6?.primaryRouter}`);
    }
    info = info.join("\n");
    return info + "\n";
  }
